...
func Test_PreprocessMaskURL_ReplacesDomainAndReturnsMapping(t *testing.T) {
	...
	assert.Equal(t, 1, len(res["mask_url"]))               // один домен
	assert.Contains(t, res["mask_url"], "google.com")      // ключ есть
	assert.Contains(t, res["mask_url"]["google.com"], ".") // значение тоже домен
}
